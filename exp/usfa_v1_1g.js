...
function readExp() {
    ...
    for (var i = 1; i <= exp.N; i++) {
        ...
        exp.adj.push(b);
        var is_term = parseInt(a[3], 10);
        exp.is_term.push(is_term);
        if (is_term) {
            exp.N_term++;
        }
    }
    ...
    // read terminal state names / images (princesses)
    exp.names_term = [];
    for (var i = 0; i < exp.N_term; i++) {
        exp.names_term.push(lines[l].trim());
        l++;
    }
...
function genExp(exp) {
    ...
    // shuffle terminal state names
    exp.names_term = shuffle(exp.names_term);
    exp.names_alt = []; // names_term but for each state
    var j = 0;
    for (var i = 0; i < exp.N; i++) {
        if (exp.is_term[i]) {
            exp.names_alt.push(exp.names_term[j]);
            j++;
        } else {
            exp.names_alt.push("");
        }
...
function redraw() {
    ...
    for (var i = 0; i < exp.D; i++) {
        ...
        sum_str += goal[i].toString() + " <img src='" + exp.feature_names[i] + "' height='20px'> x " + exp.phi[cur - 1][i].toString();
    }
    
    // show goal / prices
    $("#goal_state").html("Gifts:<br />" + goal_str_small);
